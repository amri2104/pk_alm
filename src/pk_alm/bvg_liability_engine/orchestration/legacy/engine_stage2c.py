"""Stage-2C BVG engine adapter over the generic engine.

Translates the legacy ``DynamicParameter`` (scalar | sequence) inputs for
``active_interest_rate`` and ``conversion_rate`` into :class:`RateCurve`
objects and calls the generic engine. Returns a :class:`Stage2CEngineResult`
rebuilt from the generic engine's :class:`BVGYearStep` sequence.
"""
...
from pk_alm.bvg_liability_engine.actuarial_assumptions.dynamic_parameters import (
    DynamicParameter,
    expand_parameter,
)
...
class Stage2CEngineResult:
    """Frozen container for one Stage-2C BVG engine run (legacy shape)."""
...
def run_bvg_engine_stage2c(
    ...
    """Stage-2C entry point. Adapter over the generic engine."""
    ...
    active_trajectory = expand_parameter(
        active_interest_rate, horizon_years, "active_interest_rate"
    )
    conversion_trajectory = expand_parameter(
    ...
    return Stage2CEngineResult(
        ...
        active_interest_rate_trajectory=active_trajectory,
        conversion_rate_trajectory=conversion_trajectory,
    )
