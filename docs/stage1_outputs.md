...
- No stochastic rates.
- AAL is required for integrated asset-side ACTUS work, but the protected
  Stage-1 baseline does not call the AAL Asset Engine and does not generate
  AAL service-backed cashflows. These seven CSV outputs remain the
  deterministic Stage-1 reference.
- The AAL Asset Engine and Full ALM Scenario now exist separately from these
  seven protected Stage-1 CSV outputs. `run_full_alm_scenario(...)` combines
