...
## ADR 011: AAL Server Limitations: STK DV and CSH Events

**Status:** Accepted

**Decision:** ACTUS asset mode records only real AAL server-emitted cashflow events. The implementation does not compensate for missing STK dividend (`DV`) events or CSH interest events with locally synthesized cashflows.

**Rationale:** Live probes against the public AAL reference server showed that PAM contracts with an interest-payment cycle emit contractual `IP` events plus `MD` at maturity. STK contracts can be constructed with dividend Group 1 terms and termination Group 6 terms, but the tested server emits only `TD` and no `DV`. CSH contracts can be constructed and included in the portfolio, but the tested server emits no temporal events. Treating these gaps as documented server/model limitations is more transparent than mixing server output with Python-side replacement cashflows.

**Consequence:** Asset-side ACTUS cashflows have `source="ACTUS"` only. STK market-value assumptions can still be used for year-end asset snapshots while a position is open, but they do not create local cashflow events. CSH appears as a static book-entry position in asset snapshots. Any thesis interpretation of ACTUS-mode results must state that equity dividends, real-estate income, and cash interest are not generated by the tested public AAL server.

## ADR 012: Why Stage 2B Mortality Uses EK 0105 as an Educational Input

**Status:** Accepted

**Decision:** Stage 2B adds mortality as a liability-side extension only. It affects retiree pension (`RP`) cashflows and retiree present value through expected-value weighting with survival probabilities. Retiree counts are not decremented, and asset-side AAL/ACTUS logic remains unchanged. `mortality_mode="off"` reproduces the Stage-2A population baseline for regression.
